...
const uploadImageForMap = (dispatch) => async ({ fileName, image_params }) => {
    ...
    const fileType = image_params.type;
    const { data: presignedUrl } = await mapAPI.post('/upload', { fileName, fileType }, config);
    ...
    xhr.send({ uri: formatURI, type: fileType, name: image_params.fileName })
}
